...
class WeightEncoding(nn.Module):
    def __init__(self, max_weight: int, node_dim: int):
        """
        :param max_weight: max weight of nodes
        :param node_dim: hidden dimensions of node features
        """
        super().__init__()
        self.max_weight = max_weight
        self.node_dim = node_dim
        self.w = nn.Parameter(torch.randn((max_weight, node_dim)))

    def forward(self, x: torch.Tensor, weights: torch.LongTensor) -> torch.Tensor:
        """
        :param x: node feature matrix
        :param weights: weights vector of nodes
        :return: torch.Tensor, node embeddings after Weight encoding
        """

        weights = self.decrease_to_max_value(weights.long(), self.max_weight - 1)
...
class GTAttentionHead(nn.Module):
...
class GTMultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_q: int, dim_k: int, edge_dim: int, max_path_distance: int):
        ...
        self.heads = nn.ModuleList(
            [GTAttentionHead(dim_in, dim_q, dim_k, edge_dim, max_path_distance) for _ in range(num_heads)]
        )
...
class GTEncoderLayer(nn.Module):
    def __init__(self, node_dim, edge_dim, num_heads, max_path_distance):
        ...
        self.attention = GTMultiHeadAttention(
            dim_in=node_dim,
            dim_k=node_dim,
            dim_q=node_dim,
            num_heads=num_heads,
    ...
    def forward(self,
        ...
        :param edge_paths: pairwise node paths in edge indexes
        :param ptr: batch pointer that shows graph indexes in batch of graphs
        :return: torch.Tensor, node embeddings after Graph transformer layer operations
        """
