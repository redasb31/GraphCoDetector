...
int main(void){
    ...
    rep(i,1,n-2){
        ll l1=2,r1=i+1,l2=i+2,r2=n-1;
        ll piv1[2],piv2[2];
        piv1[0]=upper_bound(sum,l1,r1,sum[i+1]/2,2);
        piv1[1]=lower_bound(sum,l1,r1,sum[i+1]/2,2);
        piv2[0]=upper_bound(sum,l2,r2,(sum[n]-sum[i+1])/2+sum[i+1],2);
        piv2[1]=lower_bound(sum,l2,r2,(sum[n]-sum[i+1])/2+sum[i+1],2);
        rep(j,0,2){
            ...
            rep(k,0,2){
                ...
                ll num[4];
                num[0]=sum[piv1[j]-1];
                num[1]=sum[i+1] - sum[piv1[j]-1];
                num[2]=sum[piv2[k]] - sum[i+1];
